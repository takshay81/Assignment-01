# Assignment-01 — TuteDude HTML Learning Demo Page

A single-page HTML demo built for TuteDude, showcasing semantic HTML5 structure — header, nav, main sections (About, Menu, Trivia, Extras), nested tables, forms, media embeds, and a footer.

## 🔗 Live Demo
[View Deployed Site](https://assignments011.netlify.app/) 

## 🛠️ Built With
- HTML5 (semantic elements: `header`, `nav`, `main`, `section`, `article`, `aside`, `figure`, `footer`, `address`, `details`)
- No frameworks, no build step

## 📂 Project Structure
```
Assignment-01/
├── index.html
└── README.md
```

## 🚀 Run Locally
1. Clone the repo:
   ```bash
   git clone https://github.com/takshay81/Assignment-01.git
   ```
2. Open `index.html` directly in your browser, or use the **Live Server** extension in VS Code.

## 📌 Author
TuteDude Learner
